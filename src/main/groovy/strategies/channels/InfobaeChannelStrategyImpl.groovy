...
class InfobaeChannelStrategyImpl extends ChannelStrategyImpl {
    ...
    List<News> getLastNews() {
        ...
        final List<HtmlAnchor> anchors = lastNewsPage.getByXPath("//a[@class='d23-feed-list-card']")
        anchors.each {anchor ->
            final String url = baseUrl + anchor.getHrefAttribute()
            final HtmlImage image = anchor.getFirstByXPath(".//img[@class='d23-feed-list-image']")
            final HtmlHeading2 h2 = anchor.getFirstByXPath(".//h2[@class='d23-feed-list-card-headline-lean']")
